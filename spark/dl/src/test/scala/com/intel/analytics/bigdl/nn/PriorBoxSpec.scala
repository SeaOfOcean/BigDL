...
val layer = PriorBox[Float](minSizes = minSizes, maxSizes = maxSizes,
  ...
  "Priorbox2" should "work" in {
    val isClip = false
    val isFlip = false
    val variances = null
    val minSizes = Array(16f)
    val maxSizes = null
    val aspectRatios = Array(0.5f)
    val stride = 1
    val layer = PriorBox[Float](minSizes = minSizes, maxSizes = maxSizes,
      _aspectRatios = aspectRatios, isFlip = isFlip, isClip = isClip,
      variances = variances, step = stride, offset = 0, imgH = 1, imgW = 1)
    val input = Tensor[Float](1, 1, 2, 2)

    val out = layer.forward(input)
    println(out.squeeze()(1).reshape(Array(8, 4)))
